...
class BoundaryConditionsModel(BaseModel):
  summary: str = ''
...
class DynamicHypothesisModel(BaseModel):
  confidence: float | None = None
  hypothesis: str
  linked_loops: list[str] = Field(default_factory=list)
  linked_variables: list[str] = Field(default_factory=list)


class CausalModelResponse(BaseModel):
  boundary_conditions: BoundaryConditionsModel = Field(default_factory=BoundaryConditionsModel)
  dynamic_hypotheses: list[DynamicHypothesisModel] = Field(default_factory=list)
  loops: list[LoopModel] = Field(default_factory=list)
  relationships: list[RelationshipModel] = Field(default_factory=list)
  variables: list[VariableModel] = Field(default_factory=list)


def create_sd_agent_v1(model: str = "gpt"):
    resolved_model = resolve_model(model)
    print(f"Creating System Dynamics Agent v1 with model: {resolved_model}")
    return create_agent(
...
- established System Dynamics principles
- logical reasoning
